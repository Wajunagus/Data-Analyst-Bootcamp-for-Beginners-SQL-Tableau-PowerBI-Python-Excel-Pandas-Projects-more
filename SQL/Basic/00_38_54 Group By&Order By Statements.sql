...
Group By, Order By
    ...
    The Group By statement is similar to Distinct in the Select statement in that it's going to show the unique values in a column, but with the difference below.
...
Select Gender, Age, COUNT(Gender) -- The COUNT(Gender) is a derived field, or a derived column. It's derived based off the Gender column, so it's technically not a real column that's in the table.
From AlexTheAnalyst..EmployeeDemographics
Group By Gender, Age
